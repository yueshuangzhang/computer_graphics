  ...
  std::function<void(Eigen::VectorXd &)> & proj_z)
{
  ...
  grad_f = [&](const Eigen::VectorXd & A)->Eigen::VectorXd
  {
    ...
    for (int i = 0; i < b.size(); i++) {
        for (int j = 0; j < 3; j++) {
          Eigen::VectorXd diff = t_tips[3 * i + j] - xb0[3 * i + j];
          // sum t_vaule
          t_value = t_value + 2 * (diff) * jacob.row(3 * i + j).transpose();
        }
    }
  ...
  proj_z = [&](Eigen::VectorXd & A)
  {
    ...
    for (int i = 0; i < skeleton.size(); i++) {
        for (int j = 0; j < 3; j++) {
          // select the min between skeleton and A, and select the max between skeleton and the result.
